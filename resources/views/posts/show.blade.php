    ...
    <div class="container col-10 my-3 p-3">
        <article>
            ...
            <h5 class="text-center">By
                <a class="text-decoration-none">{{ $post->author }}</a>
            </h5>
            <div class="container col-8">
                @if ($post->image)
                    <img class="img-fluid img-thumbnail" src="/storage/{{ $post->image }}" alt="">
                @else
                    <img class="img-fluid img-thumbnail" src="https://source.unsplash.com/random/400×600/?{{ $post->slug }}" alt="">
                @endif
            </div>
            <article class="my-3">
                {!! $post->body !!}
            ...
            <div class="row">
                ...
                <div class="col-4">
                    <a href="/admin/posts/{{ $post->id }}/edit" class="btn btn-outline-warning d-flex justify-content-center col-12">
                        <i class="bi bi-pencil-square mx-2"></i>
                        Edit
                    </a>
                </div>
                <form action="/admin/posts/{{ $post->id }}" method="POST" class="d-inline col-4">
                    @method('delete')
                    @csrf
